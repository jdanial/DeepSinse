...
function accuracy = imageAnnotatorAddFn(data,roiRadius,meanNoise)
...
particles = regionprops(thresholdImage,'centroid');

%% creating an array of all particles in an image
imageVec = zeros(roiWidth,roiWidth,1,length(particles));
for particleId = 1 : length(particles)
    try
        imageVec(:,:,1,particleId) = image(round(particles(particleId).Centroid(2)) - roiRadius :...
...
for particleId = 1 : length(particles)
    if classVec(particleId,1) == '1' && ...
            ...
            particles(particleId).Centroid(1) < size(image,1) - roiRadius - 1 && ...
            particles(particleId).Centroid(2) > 2 + roiRadius && ...
            particles(particleId).Centroid(2) < size(image,2) - roiRadius - 1
